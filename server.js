...
async function handleQQPlaylistSearch(keywords, limit) {
  ...
  if (!kw) return [];
  console.log('[QQPlaylistSearch]', kw, 'limit:', limit);

  // t: 2 代表搜索类型为歌单
  const result = await qqGetJSON('https://c.y.qq.com/soso/fcgi-bin/client_search_cp', {
    p: 1,
    n: limit || 20,
    w: kw,
    t: 2, 
  ...
  const rawList = (result && result.data && result.data.diss && result.data.diss.list) || [];
  return rawList.map(pl => ({
    provider: 'qq',
    source: 'qq',
    type: 'playlist',
    id: String(pl.dissid || ''),
    name: pl.dissname || '',
    cover: pl.imgurl || '',
    trackCount: pl.song_count || 0,
    playCount: pl.listennum || 0,
    creator: pl.creator ? pl.creator.name : 'QQ音乐用户',
    subscribed: false,
  }));
}
...
const KG_PLAYLIST_SEARCH_URL = 'http://mobilecdn.kugou.com/api/v3/search/special';
...
function mapQQPlaylist(pl, kind) {
  ...
  return {
    ...
    source: 'qq',
    id: id ? String(id) : '',
    name: pl.diss_name || pl.dissname || pl.dirname || pl.name || pl.title || '',
    cover: pl.diss_cover || pl.imgurl || pl.logo || pl.picurl || pl.cover || '',
    trackCount: pl.song_cnt || pl.songnum || pl.total_song_num || pl.song_count || pl.song_count_v2 || 0,
...
function collectQQPlaylistObjects(input, out, seen, depth) {
  out = out || [];
  seen = seen || new Set();
  depth = depth || 0;
  if (!input || depth > 6) return out;
  if (Array.isArray(input)) {
    input.forEach(item => collectQQPlaylistObjects(item, out, seen, depth + 1));
    return out;
  }
  if (typeof input !== 'object') return out;
  const id = input.dissid || input.tid || input.dirid || input.id || input.diss_id;
  const name = input.diss_name || input.dissname || input.dirname || input.name || input.title;
  if (id && name) {
    const key = String(id);
    if (!seen.has(key)) {
      seen.add(key);
  ...
  ['cdlist', 'disslist', 'dirlist', 'list', 'v_list', 'data', 'result'].forEach(key => {
    if (input[key]) collectQQPlaylistObjects(input[key], out, seen, depth + 1);
  });
  return out;
}
...
async function handleQQUserPlaylists() {
  ...
  const collected = collectRaw.status === 'fulfilled'
    ? collectQQPlaylistObjects(collectRaw.value).map(pl => mapQQPlaylist(pl, 'collect')) : [];
  const seen = new Set();
...
async function kgSearchPlaylists(keywords, limit) {
  var kw = String(keywords || '').trim();
  if (!kw) return [];
  console.log('[KGPlaylistSearch]', kw, 'limit:', limit);
  limit = Math.max(4, Math.min(30, limit || 18));
  var data = await kgMobileGet(KG_PLAYLIST_SEARCH_URL, {
    keyword: kw,
    page: '1',
    pagesize: String(limit),
    format: 'json',
  }, { cookie: false });
  var list = (data && data.data && data.data.info) || [];
  var seen = {};
  return list
    .map(function(raw) { return mapKGPlaylist(raw, '酷狗音乐'); })
    .filter(function(pl) {
      if (!pl || !pl.id || seen[pl.id]) return false;
      seen[pl.id] = true;
      return true;
    })
    .slice(0, limit);
...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- 网易云: 搜索歌单 ----------
  if (pn === '/api/playlist/search') {
    try {
      const kw = url.searchParams.get('keywords') || '';
      const limit = parseInt(url.searchParams.get('limit') || '20', 10);

      // type: 1000 为网易云歌单搜索
      const r = await cloudsearch({ keywords: kw, type: 1000, limit, cookie: userCookie });
      const playlists = (r.body && r.body.result && r.body.result.playlists) || [];

      const mapped = playlists.map(pl => mapDiscoverPlaylist(pl, '搜索结果'));
      sendJSON(res, { playlists: mapped });
    } catch (err) {
      console.error('[NeteasePlaylistSearch]', err);
  ...
  if (pn === '/api/qq/playlist/search') {
    try {
      const kw = url.searchParams.get('keywords') || '';
      const limit = parseInt(url.searchParams.get('limit') || '20', 10) || 20;
      const playlists = await handleQQPlaylistSearch(kw, limit);
      sendJSON(res, { provider: 'qq', playlists });
    } catch (err) {
      console.error('[QQPlaylistSearchRoute]', err);
  ...
  if (pn === '/api/kg/playlist/search') {
    try {
      const kw = url.searchParams.get('keywords') || '';
      const limit = Math.max(4, Math.min(24, parseInt(url.searchParams.get('limit') || '18', 10) || 18));
      const playlists = await kgSearchPlaylists(kw, limit);
      sendJSON(res, { provider: 'kugou', playlists });
    } catch (err) {
      console.error('[KGPlaylistSearch]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, playlists: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/user/playlists') {
    try {
      ...
      const limit = Math.max(12, Math.min(500, parseInt(url.searchParams.get('limit') || '300', 10) || 300));
